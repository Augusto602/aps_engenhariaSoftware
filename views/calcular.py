...
class CalcularView(tk.Frame):
    ...
    def _construir_ui(self):
        ...
        self.entrada = construir_campo(
            interno,
            "Entrada",
            "ex: 100,50"
        )

        self.transformidade = construir_campo(
            interno,
            "Transformidade",
            "ex: 2,3"
        )
    ...
    def _parse_matriz(self, texto):
        return [list(map(float, linha.split(","))) for linha in texto.split(";")]
    ...
    def calcular(self):
        try:
            ...
            self._definir_resultado(
                f"Emergia: {round(emergia, 2)}     "
                f"Fluxo: {[float(round(v, 2)) for v in fluxo]}"
            )
    ...
    def importar_completo(self):
        ...
        try:
            ...
            if len(entrada) != tamanho:
                raise Exception("Entrada incompatível")
            ...
            self.transformidade.insert(
                0,
                ",".join(map(str, transformidade))
            )
